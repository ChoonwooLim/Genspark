...
## 2. 통합 논지 — 하나의 스튜디오, 세 개의 엔진

```
              ┌─ canvas     결정론적 · 알파 · 무료 · 즉시
로고 1장 ─────┼─ seedance   시네마틱 · 유료 · 수 분
              └─ composite  Seedance 플레이트 + Canvas 알파 로고
                            ↑ 어느 쪽도 혼자서는 못 만드는 결과물
```

`composite` 가 통합의 이유다. Seedance 에게 **로고 없는 VFX 플레이트**만 시키고,
로고 레이어는 Canvas 가 진짜 알파로 렌더해 서버에서 겹친다.

- VFX 는 헐리우드급 — Seedance 가 만든다
- 로고는 벡터급 — Canvas 가 그리므로 워드마크가 절대 뭉개지지 않는다
- 착지 프레임 보정이 아예 필요 없다 — 로고가 처음부터 정확하다

2026-08-30 세션에서 이 두 조각을 각각 실제로 만들어 확인했다.
`testB_vfx_plate_5s.mp4`(로고 없는 플라즈마 플레이트)와
`plazion_transparent_1920x1080_30fps/`(이 앱이 뽑은 알파 시퀀스)가 그것이다.
합치는 단계만 없었다.
...
| **M2** | 비계 제거(핸드오프 내보내기 후 삭제), PLAZION 값 파라미터화 | 임의 브랜드로 canvas 렌더 가능 |
| **M3** | `seedance` 엔진 + 프리셋 6종 + post 체인 + QA | AI 렌더가 사이트에서 돌아감 |
| **M4** | `composite` 엔진, Canvas 레이어 모드, 타이밍 파라미터화 | 통합의 목표 기능 완성 |

M1 을 먼저 하는 이유 — 지금 `render.node.ts` 에 Playwright 가 박혀 있어 엔진을 추가할
자리가 없다. 리팩토링 없이 Seedance 를 밀어넣으면 500줄이 900줄이 된다.

M2 를 M3 앞에 두는 이유 — 프리셋 구조가 브랜드 중립화의 결과물이다. 순서를 바꾸면
PLAZION 전용 프리셋을 만들었다가 다시 뜯게 된다.
