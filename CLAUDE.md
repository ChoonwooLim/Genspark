...
정본은 **`docs/logo-studio-design.md`** (2026-08-30 2차 재설계). 새 세션은 **이 문서를
먼저 읽고** 시작한다. 제품 원칙·6단계 워크플로(Brand→Direct→Explore→Refine→Sound→
Deliver)·web/worker 아키텍처·데이터 모델·마이그레이션 P0~P6·실측 근거가 전부 거기 있다.

**P2(생성·검수)가 첫 목표다.** P0(기반)·P1(브랜드·디렉션)이 자리를 만든다.
P2 가 끝나면 멈추고 실제 브랜드로 써 본 뒤 P3 이후를 재우선순위한다.
...
오디오(효과음·음악)는 정본 §9. **Higgsfield 로는 음악·효과음을 만들 수 없다** —
