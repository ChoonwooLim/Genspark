...
- **Features**:
  ...
  - `내 로고 사용`, `새 로고 만들기`, `Genspark 결과 가져오기`를 분리한 단계형 소스 선택
  - Genspark 내부 UI에서 만든 결과의 직접 이미지 주소를 서버 프록시로 가져와 현재 로고에 자동 적용
  - Genspark AI 신규 로고 생성, 글로우·모션 에너지 조정
...
## User Guide
1. **로고 작업실** 1단계에서 `내 로고 사용`, `새 로고 만들기`, `Genspark 결과 가져오기` 중 하나를 선택합니다.
2. AI를 선택한 경우 새 브랜드 이름, 로고 구성, 색상, 스타일, 차별화 강도와 피하고 싶은 요소를 지정해 기존 PLAZION과 무관한 새 시안을 생성합니다. 생성 결과가 미리보기에 적용됐다는 확인 표시를 확인합니다.
3. Genspark 내부 UI에서 이미 만든 결과를 사용하려면 `Genspark 결과 가져오기`를 선택합니다. 결과 이미지를 복사해 가져오기 영역에서 `Ctrl+V`로 바로 적용하거나, 직접 이미지 주소를 붙여 넣습니다. 로그인 세션 전용 `/api/files/s/...` 링크는 서버에서 가져올 수 없으므로 클립보드 붙여넣기 또는 다운로드 후 업로드를 사용합니다.
4. 프로젝트 이름, 프리셋, 글로우, 모션 에너지와 16:9/9:16 화면비를 설정합니다.
5. **프로젝트 저장 + 프리셋 등록**을 눌러 로고와 설정을 저장합니다. Orbitron 저장 API가 아직 연결되지 않은 환경에서는 현재 브라우저의 IndexedDB에 안전하게 임시 저장됩니다.
6. **로고 라이브러리**에서 저장 프로젝트를 검색하고 불러오기, 원본 다운로드, 시퀀스 출력 또는 삭제를 실행합니다.
7. **폴더에 PNG 시퀀스 저장** 버튼을 누르고 데스크톱 또는 원하는 위치를 선택합니다.
8. 선택한 위치에 `plazion_transparent_[해상도]_30fps` 하위 폴더가 생성되고 `plazion_0000.png`부터 `plazion_0089.png`까지 저장됩니다.
9. Chrome/Edge 외 브라우저처럼 폴더 저장 API가 없는 환경에서는 동일한 파일이 ZIP으로 다운로드됩니다.
...
- `POST /api/ai/import-genspark-image`: 허용된 `https://*.genspark.ai` 직접 이미지 URL을 검증·다운로드해 브라우저에 이미지 바이너리로 전달
- `?genspark_image=<URL>`로 사이트를 열면 가져오기 탭을 선택하고 해당 결과 URL을 자동으로 불러오기 시도
