...
- **Features**:
  ...
  - 로고 업로드/드래그앤드롭, Genspark AI 로고 생성, 글로우·모션 에너지 조정
...
## User Guide
1. **로고 작업실**에서 PNG/SVG/WEBP/JPG 파일을 업로드하거나 브랜드 이름·설명을 입력해 Genspark AI로 새 로고를 생성합니다.
2. 프로젝트 이름, 프리셋, 글로우, 모션 에너지와 16:9/9:16 화면비를 설정합니다.
3. **프로젝트 저장 + 프리셋 등록**을 눌러 로고와 설정을 저장합니다. Orbitron 저장 API가 아직 연결되지 않은 환경에서는 현재 브라우저의 IndexedDB에 안전하게 임시 저장됩니다.
4. **로고 라이브러리**에서 저장 프로젝트를 검색하고 불러오기, 원본 다운로드, 시퀀스 출력 또는 삭제를 실행합니다.
5. **폴더에 PNG 시퀀스 저장** 버튼을 누르고 데스크톱 또는 원하는 위치를 선택합니다.
6. 선택한 위치에 `plazion_transparent_[해상도]_30fps` 하위 폴더가 생성되고 `plazion_0000.png`부터 `plazion_0089.png`까지 저장됩니다.
7. Chrome/Edge 외 브라우저처럼 폴더 저장 API가 없는 환경에서는 동일한 파일이 ZIP으로 다운로드됩니다.
